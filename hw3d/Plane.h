...
public:
	...
	static IndexedTriangleList MakeTesselatedQuad(Dvtx::VertexLayout layout, int divisions_x, int divisions_y)
	{
		namespace dx = DirectX;
		assert(divisions_x >= 1);
		assert(divisions_y >= 1);
		...
		std::vector<unsigned short> indices;
		indices.reserve(sq(divisions_x * divisions_y) * 4);
		{
			const auto vxy2i = [nVertices_x](size_t x, size_t y)
			{
				return (unsigned short)(y * nVertices_x + x);
	...
	static IndexedTriangleList Make(char type = 'D')
	{
		...
		{
		case 'T':
			return MakeTessellation(false, false, 1);
		case 'F':
		...
		bool isQuad = false)
	{
		...
		if (isQuad)
		{
			return MakeTesselatedQuad(std::move(vl), tessellation, tessellation);
		}
		else
		{
